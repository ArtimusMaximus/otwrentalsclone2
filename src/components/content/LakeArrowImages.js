...
export default () => {
    ...
    return (
      <>
      <div className="row text-center text-lg-start col-6">
        {arr.map((image, i) => {
            return (
            <div key={i}>
                <a className='d-block h-100' onClick={() => {setShowModal(true); setModalData(image)}}>
                    <img className="img-fluid img-thumbnail" src={image} style={{height: '50px', width: '75px', margin: '5px', borderRadius: '5px'}} />
                </a>
            </div>
            )
        })}
